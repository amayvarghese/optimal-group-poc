...
export function GridPixelateWipe({
  ...
  const start =
    typeof transitionStart === 'number' ? transitionStart : Math.floor(durationInFrames * 0.22)
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      const opacity =
        frame < start
          ? 1
          : interpolate(frame, [start + delay, start + delay + cellFadeFrames], [1, 0], {
  ...
  return (
    <div className={className} style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: 'black' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, opacity: frame < start ? 1 : 0 }}>{from}</div>
    </div>
  )
}
